...
flist_log=dir('*.log'); 
for i=1:length(flist_log), fnum_log(i) = str2num(flist_log(i).name(5:end-4)); end
flist_smr=dir('*.smr');
for i=1:length(flist_smr), fnum_smr(i) = str2num(flist_smr(i).name(5:end-4)); end
% flist_mat=dir('*.mat');
...
for i=1:nfiles
    ...
    for j = indx_smr
        ...
        %         data_smr = (flist_smr(j).name);
        %         data_smr = ImportSMR2(flist_smr(j).name);
